...
subtitle: "R Tidyverse - Project Workflow and Report"
date: today
author: 
...
cat("USDA_ATLAS shape(rows x columns):", nrow(usda_atlas), "\n")
cat("County_Demographics shape(rows x columns):", nrow(county_demographics), "\n")
cat("Food_Nutrition shape(rows x columns):", nrow(food_nutrition), "\n")
```
...
usda_texas_subset <- usda_texas %>%
  ...
  rename(
    LowAccess_Pop = 'lapop1',
    LowIncome_LowAccess = 'lalowi1',
    Black_LowAccess = 'lablack1',
    Hispanic_LowAccess = 'lahisp1',
    Tract_LowIncome = 'TractLOWI'
  )
```
...
```{r}
cat("USDA subset shape:", paste(nrow(usda_texas_subset), collapse = " x "))
```
...
```{r}
# We use backticks because column names contain spaces and dots.
# Select and Rename 10 columns
...
```{r}
cat("Demographics subset shape:", paste(nrow(county_demographics_subset), collapse = " x "))
```
...
```{r}
cat("Food subset shape:", paste(nrow(food_nutrition_subset), collapse = " x "))
```
...
usda_texas_cleaned <- usda_texas_subset %>%
  mutate(
    LowAccess_Pop = as.numeric(LowAccess_Pop),
    LowIncome_LowAccess = as.numeric(LowIncome_LowAccess),
    Black_LowAccess = as.numeric(Black_LowAccess),
    Hispanic_LowAccess = as.numeric(Hispanic_LowAccess)
  ) %>%
  drop_na()
```
```{r}
...
cat("Zeros in Tract_LowIncome:", sum(usda_texas_cleaned$Tract_LowIncome == 0), "\n")

usda_texas_cleaned <- usda_texas_cleaned %>%
  filter(!(Tract_LowIncome == 0 & LowIncome_LowAccess == 0))

cat(paste("Find", nrow(usda_texas_cleaned), "rows data are not both zero：\n"))

print(usda_texas_cleaned)
```
...
print(paste("Unique counties in Demographics:", n_distinct(county_demographics_cleaned$County)))
```
...
usda_with_metrics <- usda_texas_cleaned %>%
  mutate(
    PercentLowAccess = (LowAccess_Pop / Pop2010) * 100,
  )
```
...
usda_with_metrics <- usda_texas_cleaned %>%
  mutate(
    PercentLowIncomeLowAccess = (LowIncome_LowAccess / Tract_LowIncome) * 100
  )
```
...
```{r}
food_nutrition_grouped <- food_nutrition_subset %>%
  group_by(Category) %>%
  summarize(
...
# Verification
print(paste("Final Food Nutrition shape:", paste(nrow(food_nutrition_final), collapse = " x ")))
...
- county_demographics_cleaned - cleaned demographics with renamed columns
...
usda_county_agg <- usda_with_metrics %>%
  ...
  summarize(
    ...
    TotalLowAccessPop = sum(LowAccess_Pop),
    TotalLowIncomePop = sum(Tract_LowIncome),
    TotalLowIncomeLowAccessPop = sum(LowIncome_LowAccess),
    TotalBlackLowAccess = sum(Black_LowAccess),
    TotalHispanicLowAccess = sum(Hispanic_LowAccess),
...
```{r}
print(paste("Weighted poverty shape:", paste(nrow(poverty_weighted), collapse = " x ")))
...
```{r}
# We use to filter to pick up rows that are reasonable

usda_county_agg_cleaned <- usda_county_agg %>%
  filter(TotalLowAccessPop < TotalPopulation, 
         TotalLowIncomeLowAccessPop < TotalLowIncomePop)

print(paste("Original rows:", nrow(usda_county_agg)))
print(paste("After removing unreasonable rows:", nrow(usda_county_agg_cleaned)))
print(paste("Rows removed:", nrow(usda_county_agg) - nrow(usda_county_agg_cleaned)))
```
...
```{r}
print(paste("Final shape:", paste(nrow(usda_county_agg_final), collapse = " x ")))
...
```{r}
county_demo_final <- county_demographics_cleaned

print(paste("County Demographics shape:", paste(nrow(county_demo_final), collapse = " x ")))
```
...
```{r}
print(paste("Merged Shape:", paste(nrow(texas_county_merged), collapse = " x ")))
...
Final Verification
```{r}
print(paste("Final shape:", paste(nrow(texas_county_final), collapse = " x ")))
...
```{r}
print(paste("Shape:", paste(nrow(texas_county_final), collapse = " x ")))
...
```{r}
print(paste("Shape:", paste(nrow(food_nutrition_final), collapse = " x ")))
...
Working through this three dataset food accessibility project presented a steady stream of challenges, from initial data exploration through the final merge operations. Though the individual steps weren't revolutionary, the cumulative effect of working with three large, messy datasets taught us that data wrangling is about systematic problem-solving.
...
We learned the Tidyverse, especially focusing on the pipe operator (%>%) for chaining data transformation steps. We referred to the package documentation to understand the difference between inner_join and other join types to ensure we didn't lose county data during merging. A key learning moment was converting "wide" nutrition data into a "long" format. We explored the tidyr documentation to implement ```pivot_longer()```, which was crucial for categorizing nutrients. Additionally, we learned ggplot2 for visualization. We searched the internet for guidance on adding linear regression layers (geom_smooth) and customizing axis labels, which allowed me to create clear, publication-ready charts comparing income and food access.
